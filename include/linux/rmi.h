...
/*Modified  macro as user read write only */
#define RMI_RW_ATTR (S_IRUSR|S_IWUSR|S_IRGRP|S_IROTH)
#define RMI_WU_ATTR S_IWUSR
...
struct rmi_device_platform_data {
	...
	int attn_gpio;
	int reset_gpio;//yangzicheng add
	...
	int (*hardware_reset)(struct rmi_device_platform_data *pdata);//yangzicheng add
...
struct rmi_phys_device {
	struct device *dev;
	struct rmi_device *rmi_dev;
       struct rmi_device_platform_data *platform_data;//yangzicheng add
...
#define to_rmi_platform_data(d) ((d)->phys->platform_data);//yangzicheng add
