...
struct kobject *rmi_virtual_key_properties_kobj;
static ssize_t
rmi_virtual_keys_register(struct kobject *kobj,
			     struct kobj_attribute *attr,
			     char *buf)
{
	return snprintf(buf, 200,
		__stringify(EV_KEY) ":" __stringify(KEY_HOME)  ":240:900:120:60"
		":" __stringify(EV_KEY) ":" __stringify(KEY_MENU)   ":405:900:120:60"
		":" __stringify(EV_KEY) ":" __stringify(KEY_BACK) ":85:900:120:60"
		"\n");
}
static struct kobj_attribute rmi_virtual_keys_attr = {
	.attr = {
		.name = "virtualkeys.synaptics",
		.mode = S_IRUGO,
	},
	.show = &rmi_virtual_keys_register,
};

static struct attribute *rmi_virtual_key_properties_attrs[] = {
	&rmi_virtual_keys_attr.attr,
	NULL,
};

static struct attribute_group rmi_virtual_key_properties_attr_group = {
	.attrs = rmi_virtual_key_properties_attrs,
};
...
static int rmi_driver_probe(struct rmi_device *rmi_dev)
{
	...
	/*creat virtual key*/
	rmi_virtual_key_properties_kobj =
			kobject_create_and_add("board_properties", NULL);

	if (rmi_virtual_key_properties_kobj)
		error = sysfs_create_group(rmi_virtual_key_properties_kobj,
				&rmi_virtual_key_properties_attr_group);
 	if (!rmi_virtual_key_properties_kobj || error)
                pr_err("failed to create synaptics board_properties\n");
