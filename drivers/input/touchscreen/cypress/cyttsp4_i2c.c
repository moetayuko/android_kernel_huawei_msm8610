	...
	const struct i2c_device_id *i2c_id)
{
	...
	unsigned int  enable_power_gpio = 0;
	...
	rc = of_property_read_u32(dev->of_node,"cy,enable_power_gpio", &enable_power_gpio);
	if (rc) {
		tp_log_err("%s: of_property_read_u32 read enable power gpio err rc=%d\n", __func__, rc);
	}
	...
	if (gpio_is_valid(enable_power_gpio) && (0 != enable_power_gpio))
	{
		/* configure touchscreen gpio to switch on 2.8V power */
		rc = gpio_request(enable_power_gpio,"cyttsp_enable_power_gpio");
		if (rc)
		{
			tp_log_err("%s: unable to request gpio [%d]\n", __func__, enable_power_gpio);
			return rc;
		} 
		else
		{
			rc = gpio_tlmm_config(GPIO_CFG(enable_power_gpio,0,
			...
			rc = gpio_direction_output(enable_power_gpio, 1);
			if (rc)
			{
