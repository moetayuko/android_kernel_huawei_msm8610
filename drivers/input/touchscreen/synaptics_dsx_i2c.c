...
#ifdef CONFIG_HUAWEI_KERNEL
			tp_log_debug("%s: x_lcd = %d,y_lcd_all = %d  ",__func__, x_lcd,y_lcd_all);
			tp_log_debug("%s: max_x = %d,max_y = %d  ",__func__, rmi4_data->sensor_max_x,rmi4_data->sensor_max_y);
...
#endif//huawei 11-25
#ifndef TYPE_B_PROTOCOL
		tp_log_debug("%s:NO 2-6-38",__func__);
		input_mt_sync(rmi4_data->input_dev);
#endif
	}
...
#endif //huawei 11-25
{
	...
	tp_log_debug("%s: data[0] = %d ,retval = %d",__func__,data[0],retval);
	...
	{
		tp_log_debug("%s:### irq_enabled! ",__func__);
		...
		bool enable)
{
	...
	tp_log_debug("%s: irq_enabled = %d ",__func__,rmi4_data->irq_enabled);
	if (enable) {
		if (rmi4_data->irq_enabled)
			return retval;
...
static int synaptics_rmi4_parse_dt(struct device *dev, struct synaptics_dsx_platform_data *pdata)
{
	...
	err = of_property_read_string(np, "synaptics,firmware_name",&pdata->firmware_name);
	tp_log_err("### Read firmware_name,err=%d \n",err); 		
	if (err){
		tp_log_err("Unable to read firmware_name\n");			
		pdata->firmware_name = "no_firmware";			
	}
#endif /*CONFIG_HUAWEI_KERNEL*/
...
static UNIVERSAL_DEV_PM_OPS(rmi_driver_pm, synaptics_rmi4_suspend,
