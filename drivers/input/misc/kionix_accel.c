...
******************************************************************************/
static short akmd_kionix_xyz[3] = { 0 }; //the value of this array is reported to hal & akmd
...
{
       printk("kionix_release\n");
	return 0;
}
static long kionix_ioctl(struct file *file, unsigned int cmd,
		   unsigned long arg)
{
	...
	case  ECS_IOCTL_READ_ACCEL_XYZ:
	        if (copy_to_user(argp, &akmd_kionix_xyz, sizeof(akmd_kionix_xyz))) 
		{
	...
	if(atomic_read(&acceld->accel_enabled) > 0) {
		if(atomic_read(&acceld->accel_enable_resume) > 0)
		{
			...
			if (err < 0) {
				...
				acceld->accel_data[acceld->axis_map_y] = -(acceld->negate_y ? -y : y) + acceld->accel_cali[acceld->axis_map_y];
				acceld->accel_data[acceld->axis_map_z] = -(acceld->negate_z ? -z : z) + acceld->accel_cali[acceld->axis_map_z];
                akmd_kionix_xyz[0] = (short)acceld->accel_data[acceld->axis_map_x];
				akmd_kionix_xyz[1] = (short)acceld->accel_data[acceld->axis_map_y];
				akmd_kionix_xyz[2] = (short)acceld->accel_data[acceld->axis_map_z];
				if(atomic_read(&acceld->accel_input_event) > 0) {
					input_report_abs(acceld->input_dev, ABS_X, acceld->accel_data[acceld->axis_map_x]);
					input_report_abs(acceld->input_dev, ABS_Y, acceld->accel_data[acceld->axis_map_y]);
					input_report_abs(acceld->input_dev, ABS_Z, acceld->accel_data[acceld->axis_map_z]);
				 ...
				 const struct i2c_device_id *id)
{
	...
	switch (err) {
		...
		case KIONIX_ACCEL_WHO_AM_I_KXCJK_1013:
			...
			break;
		case KIONIX_ACCEL_WHO_AM_I_KX023:
			KMSGINF(&client->dev, "%s %d:get in KIONIX_ACCEL_WHO_AM_I_KX023\n",
			            __func__, __LINE__);
            err = app_info_set("G-Sensor", "kionix_KX023-1025");
			if (err < 0)/*failed to add app_info*/
			{
				KMSGERR(&client->dev, "%s %d:faile to add app_info\n", __func__, __LINE__);
			}
	...
	if (err) {
		printk(KERN_ERR "kionix_device_probe: "
			   "kionix_device register failed\n");
		goto err_free_accel_registers;
	}
	atomic_set(&acceld->accel_suspended, 0);
	atomic_set(&acceld->accel_suspend_continue, 1);
