...
#if defined MSM_8612_PLATFORM
#define RECORD_PHONE "record on msm 8612 platform"
#elif defined MSM_8226_PLATFORM
...
static int inputevent_data_save(void)
{
    ...
    set_fs(KERNEL_DS);
    pos = 0;
    snprintf(buf, IE_SIZE, "%lu", ate_dt->ie_times);
    vfs_write(fp, buf, sizeof(buf), &pos);
    memset(buf, 0, IE_SIZE);
    snprintf(buf, IE_SIZE, "%lu", ate_dt->playback_continuance_ms);
    vfs_write(fp, buf, sizeof(buf), &pos);
...
{
#if defined MSM_8612_PLATFORM
    strncpy(ate_dt->idf.dev_name[0], "cyttsp4_mt", strlen("cyttsp4_mt")+1);
    ate_dt->idf.input_event_type[0] = TOUCH_EVENT;
    strncpy(ate_dt->idf.dev_name[1], "qpnp_pon", strlen("qpnp_pon")+1);
    ate_dt->idf.input_event_type[1] = POWER_KEY_EVENT;
    strncpy(ate_dt->idf.dev_name[2], "gpio-keys", strlen("gpio-keys")+1);
    ...
    strncpy(ate_dt->idf.dev_name[2], "gpio-keys", strlen("gpio-keys")+1);
    ate_dt->idf.input_event_type[2] = SOUND_KEY_EVENT;
    strncpy(ate_dt->idf.dev_name[3], "msm8226-tapan-snd-card Button Jack", strlen("msm8226-tapan-snd-card Button Jack")+1);
    ate_dt->idf.input_event_type[3] = NULL_EVENT_TYPE;
    strncpy(ate_dt->idf.dev_name[3], "msm8226-tapan-snd-card Headset Jack", strlen("msm8226-tapan-snd-card Headset Jack")+1);
    ate_dt->idf.input_event_type[3] = NULL_EVENT_TYPE;
    ate_dt->idf.filer_total = SOUND_KEY_EVENT+1;
#elif defined MSM_8960_PLATFORM
    strncpy(ate_dt->idf.dev_name[0], "synaptics", strlen("synaptics")+1);
    ate_dt->idf.input_event_type[0] = TOUCH_EVENT;
    strncpy(ate_dt->idf.dev_name[1], "qt1060", strlen("qt1060")+1);
    ate_dt->idf.input_event_type[1] = TOUCH_BUTTON_EVENT;
    strncpy(ate_dt->idf.dev_name[2], "pmic8xxx_pwrkey", strlen("pmic8xxx_pwrkey")+1);
    ate_dt->idf.input_event_type[2] = POWER_KEY_EVENT;
    strncpy(ate_dt->idf.dev_name[3], "gpio-keys", strlen("gpio-keys")+1);
    ate_dt->idf.input_event_type[3] = SOUND_KEY_EVENT;
    ate_dt->idf.filer_total = SOUND_KEY_EVENT+1;
#elif defined TI_OMAP_PLATFORM
    strncpy(ate_dt->idf.dev_name[0], "atmel-touchscreen", strlen("atmel-touchscreen")+1);
    ate_dt->idf.input_event_type[0] = TOUCH_EVENT;
    strncpy(ate_dt->idf.dev_name[1], "qt1060", strlen("qt1060")+1);
    ate_dt->idf.input_event_type[1] = TOUCH_BUTTON_EVENT;
    strncpy(ate_dt->idf.dev_name[2], "twl6030_pwrbutton", strlen("twl6030_pwrbutton")+1);
    ate_dt->idf.input_event_type[2] = POWER_KEY_EVENT;
    strncpy(ate_dt->idf.dev_name[3], "huawei_keypad", strlen("huawei_keypad")+1);
    ate_dt->idf.input_event_type[3] = SOUND_KEY_EVENT;
    ate_dt->idf.filer_total = SOUND_KEY_EVENT+1;
    ...
    strncpy(ate_dt->idf.dev_name[1], "qt1060", strlen("qt1060")+1);
    ate_dt->idf.input_event_type[1] = TOUCH_BUTTON_EVENT;
    strncpy(ate_dt->idf.dev_name[2], "k3v2_power_key", strlen("k3v2_power_key")+1);
    ate_dt->idf.input_event_type[2] = POWER_KEY_EVENT;
    strncpy(ate_dt->idf.dev_name[3], "k3v2_gpio_key", strlen("k3v2_gpio_key")+1);
    ate_dt->idf.input_event_type[3] = SOUND_KEY_EVENT;
    ate_dt->idf.filer_total = SOUND_KEY_EVENT+1;
#else
    strncpy(ate_dt->idf.dev_name[0], "synaptics", strlen("synaptics")+1);
    ...
    strncpy(ate_dt->idf.dev_name[1], "qt1060", strlen("qt1060")+1);
    ate_dt->idf.input_event_type[1] = TOUCH_BUTTON_EVENT;
    strncpy(ate_dt->idf.dev_name[2], "7k_handset", strlen("7k_handset")+1);
    ate_dt->idf.input_event_type[2] = POWER_KEY_EVENT;
    strncpy(ate_dt->idf.dev_name[3], "default_keypad", strlen("default_keypad")+1);
    ate_dt->idf.input_event_type[3] = SOUND_KEY_EVENT;
    ate_dt->idf.filer_total = SOUND_KEY_EVENT+1;
#endif
}
...
static int __init ate_init(void)
{
    ...
    strncpy(ate_dt->iedt_file_dir, INPUT_EVENTS_DATA_FILE_DEFAULT, MAX_FILE_DIR_NAME);
