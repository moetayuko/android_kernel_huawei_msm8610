...
static int32_t s5k4e1_sunny_sensor_enable_i2c_mux(struct msm_camera_i2c_conf *i2c_conf)
{
...
static int32_t s5k4e1_sunny_sensor_disable_i2c_mux(struct msm_camera_i2c_conf *i2c_conf)
{
	struct v4l2_subdev *i2c_mux_sd =
		dev_get_drvdata(&i2c_conf->mux_dev->dev);
	v4l2_subdev_call(i2c_mux_sd, core, ioctl,
...
int32_t s5k4e1_sunny_sensor_power_up(struct msm_sensor_ctrl_t *s_ctrl)
{
	...
	for (index = 0; index < power_setting_array->size; index++) {
		...
		case SENSOR_I2C_MUX:
			if (data->i2c_conf && data->i2c_conf->use_i2c_mux)
				s5k4e1_sunny_sensor_enable_i2c_mux(data->i2c_conf);
			break;
		default:
...
power_up_failed:
	...
	for (index = 0; index < power_setting_array->size; index++){
		...
		case SENSOR_I2C_MUX:
			if (data->i2c_conf && data->i2c_conf->use_i2c_mux)
				s5k4e1_sunny_sensor_disable_i2c_mux(data->i2c_conf);
			break;
		default:
			pr_err("%s error power seq type %d\n", __func__,
				power_setting->seq_type);
...
int32_t s5k4e1_sunny_sensor_power_down(struct msm_sensor_ctrl_t *s_ctrl)
{
	int32_t index = 0;
	struct msm_sensor_power_setting_array *power_setting_array = NULL;
	struct msm_sensor_power_setting *power_setting = NULL;
...
static struct msm_sensor_fn_t s5k4e1_sunny_sensor_func_tbl = {
	...
	.sensor_power_up = s5k4e1_sunny_sensor_power_up,
	.sensor_power_down = s5k4e1_sunny_sensor_power_down,
...
#endif
#ifdef CONFIG_HUAWEI_KERNEL_CAMERA
	.power_down_setting_array.power_setting = s5k4e1_sunny_power_down_setting,
	.power_down_setting_array.size = ARRAY_SIZE(s5k4e1_sunny_power_down_setting),
#endif
};
